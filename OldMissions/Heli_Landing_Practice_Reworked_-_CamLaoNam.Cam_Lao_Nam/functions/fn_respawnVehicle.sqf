/*
	...
	Added Fix from user Muzzleflash (https://feedback.bistudio.com/T123940#1643675) in Line 396 - KiloSwiss
	Changed hint to be shown to all players, no matter which SIDE in Line 445 - KiloSwiss
...
if (typeof _logic == "Logic" /*"ModuleRespawnVehicle_F"*/) then
{
	...
	if (_activated) then
	{
...

		{
			...
			{
				[_veh,_delay,_desertedDelay,_respawnCount,_init,_position,_positionType,_wreck,_showNotification,_forcedRespawn,_desertedDistance, _respawnWhenDisabled] call KS_fnc_respawnVehicle;
			};
		}
...
else
{
	...
	_showNotification		= _this param [8,1,[0,true]];
			...
			_mpkilled = _veh addmpeventhandler [
				"mpkilled",
				{
					...
					if (isserver) then
					{
						...
						_veh setVariable ["BIS_fnc_moduleRespawnVehicle_respawnScript", ([_veh] + _data + [_killer]) spawn KS_fnc_respawnVehicle];
					};
				}
			];
							...
							if ({alive _x} count crew _veh == 0 && (_desertedDelay > 0 || _desertedDistance > 0)) then {
								...
								_desertedScript = [_veh,_desertedDelay,_desertedDistance] spawn {
									...
									if (alive _veh && {alive _x} count crew _veh == 0) then {
										...
										_data = _veh getvariable ["BIS_fnc_moduleRespawnVehicle_data",[]];
										([_veh] + _data) call KS_fnc_respawnVehicle;
									};
								};
	...
	else
	{
		...
		if (count _respawnPos > 0) then {
			...
			//--- Fix for vehicles only respawning once. See comment from Muzzleflash: https://feedback.bistudio.com/T123940
			...
			if (_respawnCount != 0) then {
				_forcedRespawn = 0;
				[_newVeh,_delay,_desertedDelay,_respawnCount,_init,_position,_positionType,_wreck,_showNotification,_forcedRespawn,_desertedDistance, _respawnWhenDisabled] spawn KS_fnc_respawnVehicle;
			};
			...
			if (_showNotification > 0) then {
				...
				["RespawnVehicle",[_displayName,_respawnName,_picture]] remoteExecCall ["BIS_fnc_showNotification", 0]; //-- Change made for this mission specifically, to show notification to all players.
			};
